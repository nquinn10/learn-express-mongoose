...
// runs asynchronously without interfering with main thread
exports.show_home = async function (res) {
  let books = await Book.countDocuments({});
  let copies = await BookInstance.countDocuments({});
  let available = await BookInstance.countDocuments({status: 'Available'}); // count only documents with specified status
